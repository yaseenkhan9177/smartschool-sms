...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto mb-6 flex justify-between items-center print:hidden">
        <a href="{{ route('admin.certificates.create') }}" class="text-gray-500 hover:text-gray-700 flex items-center gap-2">
            <i class="fa-solid fa-arrow-left"></i> Back to Selection
        </a>
        <div class="flex gap-3">
            <button onclick="window.print()" class="px-5 py-2.5 bg-gray-800 text-white rounded-xl font-medium hover:bg-gray-900 shadow-lg flex items-center gap-2">
            ...
            <form action="{{ route('admin.certificates.store') }}" method="POST">
                ...
                <button type="submit" class="px-5 py-2.5 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 shadow-xl shadow-green-600/20 flex items-center gap-2">
                    <i class="fa-solid fa-check-circle"></i> Issue & Save
                </button>
            </form>
        </div>
    </div>
    ...
    <div class="certificate-container bg-white shadow-2xl mx-auto relative overflow-hidden print:shadow-none print:w-full print:border-none">
        ...
        <div class="text-center pt-16 pb-8">
            {{-- Check if school has logo, else generic --}}
            <div class="w-24 h-24 bg-gray-100 rounded-full mx-auto mb-4 flex items-center justify-center">
                <i class="fa-solid fa-school text-4xl text-gray-400"></i>
            </div>

            <h1 class="text-4xl font-serif font-bold text-gray-900 tracking-wide uppercase mb-2">{{ $template->title }}</h1>
        ...
        <div class="px-20 pt-16 pb-20 flex justify-between items-end mt-auto">
            <div class="text-center">
                ...
                <p class="font-bold text-gray-900 uppercase text-sm">{{ $template->footer_left }}</p>
            </div>
            ...
            <div class="text-center">
                <div class="w-48 border-b-2 border-gray-800 mb-2"></div>
                <p class="font-bold text-gray-900 uppercase text-sm">{{ $template->footer_right }}</p>
                <p class="text-xs text-gray-500 mt-1">{{ $issueDate }}</p>
            </div>
        </div>
