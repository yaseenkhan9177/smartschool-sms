...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('accountant.fees.collect.index') }}" class="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors">
            Back to List
        </a>
    </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-800">
                            ...
                            @else
                            <form id="update-form-{{ $fee->id }}" action="{{ route('accountant.fees.item.update', $fee->id) }}" method="POST" class="flex items-center space-x-2" onsubmit="event.preventDefault(); confirmUpdate('{{ $fee->id }}')">
                                @csrf
                                @method('PUT')
                        ...
                        <td class="px-6 py-4 text-center">
                            ...
                            @else
                            <form id="delete-form-{{ $fee->id }}" action="{{ route('accountant.fees.item.remove', $fee->id) }}" method="POST" onsubmit="event.preventDefault(); confirmDelete('{{ $fee->id }}')" class="inline-block">
                                @csrf
                                <!-- Using existing route/method which might be GET or POST based on web.php, let's assume POST or DELETE -->
                                <!-- Checking web.php or controller instruction: removeInvoiceItem($id). Usually needs a route. -->
