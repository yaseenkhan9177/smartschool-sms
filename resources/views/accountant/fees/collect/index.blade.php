...
<div class="space-y-6" x-data="{ 
    ...
    <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
        <form action="{{ route('accountant.fees.collect.index') }}" method="GET" class="flex flex-wrap gap-4 items-end">
            <div>
    ...
    <!-- Bulk Print Form -->
    <form action="{{ route('accountant.fees.bulk-print') }}" method="POST" target="_blank" id="bulkPrintForm">
        @csrf
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors group">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-sm font-medium text-gray-900">
                                            ...
                                            <a href="{{ route('accountant.fees.invoice', $invoice->sub_fees->first()->id) }}" target="_blank" class="text-xs text-purple-600 hover:text-purple-800 ml-1" title="View Invoice">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    ...
                                    <a href="{{ route('accountant.fees.edit', $invoice->invoice_no ?: 'id-' . $invoice->sub_fees->first()->id) }}" class="px-3 py-1.5 bg-blue-50 text-blue-600 border border-blue-200 text-xs font-medium rounded-lg hover:bg-blue-100 transition-colors shadow-sm flex items-center gap-1.5">
                                        <i class="fa-solid fa-pen-to-square"></i> Edit
                                    </a>

                                    @if($invoice->status == 'paid' || $invoice->status == 'partial')
                                    <a href="{{ route('accountant.fees.invoice', $invoice->sub_fees->first()->id) }}" target="_blank" class="px-3 py-1.5 bg-green-50 text-green-600 border border-green-200 text-xs font-medium rounded-lg hover:bg-green-100 transition-colors shadow-sm flex items-center gap-1.5">
    ...
    <div x-show="showCollectModal" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form :action="'{{ route('accountant.fees.collect.store', '') }}/' + selectedInvoiceNo" method="POST">
                    @csrf
...
<div id="generateModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <form action="{{ route('accountant.fees.generate') }}" method="POST">
                @csrf
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
...
<div id="generateTransportModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title-transport" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
        <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <form action="{{ route('accountant.transport.fees.generate') }}" method="POST">
                @csrf
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
