...
<div class="container mx-auto px-4 py-8">
    <!-- Back Link -->
    <a href="{{ route('admin.students') }}" class="inline-flex items-center gap-2 text-slate-500 hover:text-blue-600 transition-colors mb-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden max-w-4xl mx-auto">
        <div class="px-8 py-6 border-b border-gray-100 bg-gray-50/50">
            <h2 class="text-xl font-bold text-gray-900">Register New Student</h2>
        ...
        <form action="{{ route('admin.students.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-6 mt-2">
                            <label class="inline-flex items-center">
                                <input type="radio" name="gender" value="male" class="form-radio text-blue-600 h-5 w-5" {{ old('gender') == 'male' ? 'checked' : '' }} required>
                                <span class="ml-2 text-gray-700">Male</span>
                            </label>
                            <label class="inline-flex items-center">
                                <input type="radio" name="gender" value="female" class="form-radio text-pink-600 h-5 w-5" {{ old('gender') == 'female' ? 'checked' : '' }}>
                    ...
                    <!-- Transport Section -->
                    <div class="md:col-span-2 p-6 bg-blue-50/50 rounded-xl border border-blue-100">
                        <h4 class="text-sm font-bold text-slate-700 uppercase tracking-wider mb-4">Transport Information (Optional)</h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <!-- Required Toggle -->
                            <div>
                                <label for="transport_required" class="block text-sm font-medium text-gray-700 mb-2">Transport Required?</label>
                                <select name="transport_required" id="transport_required" class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 shadow-sm" onchange="toggleTransportFields()">
                                    <option value="no">No</option>
                                    <option value="yes" {{ old('transport_required') == 'yes' ? 'selected' : '' }}>Yes</option>
                                </select>
                            </div>
                        </div>

                        <!-- Dynamic Fields Wrapper -->
                        <div id="transport_fields_wrapper" class="hidden mt-6">
                            <div id="transport_fields" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <!-- Route Selection -->
                                <div>
                                    <label for="transport_route_id" class="block text-sm font-medium text-gray-700 mb-2">Select Route <span class="text-red-500">*</span></label>
                                    <select name="transport_route_id" id="transport_route_id" class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 shadow-sm" onchange="updateTransportFee(this)">
                                        <option value="">Select Route</option>
                                        @foreach($transportRoutes as $route)
                                        <option value="{{ $route->id }}" data-fee="{{ $route->monthly_fee }}">{{ $route->route_name }} (PKR {{ number_format($route->monthly_fee, 0) }})</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <!-- Pickup Point -->
                                <div>
                                    <label for="pickup_point" class="block text-sm font-medium text-gray-700 mb-2">Pickup Point</label>
                                    <input type="text" name="pickup_point" id="pickup_point"
                                        class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 shadow-sm"
                                        placeholder="e.g. Mingora Chowk" value="{{ old('pickup_point') }}">
                                </div>

                                <!-- Monthly Fee -->
                                <div>
                                    <label for="transport_fee" class="block text-sm font-medium text-gray-700 mb-2">Monthly Fee</label>
                                    <div class="relative">
                                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">PKR</span>
                                        <input type="number" name="transport_fee" id="transport_fee" min="0" step="0.01"
                                            class="w-full pl-12 rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 shadow-sm"
                                            placeholder="0" value="{{ old('transport_fee') }}">
                                    </div>
                                    <p class="text-xs text-gray-500 mt-1">Auto-filled from route. Edit if custom discount applies.</p>
                                </div>

                                <!-- Start Month -->
                                <div>
                                    <label for="transport_start_month" class="block text-sm font-medium text-gray-700 mb-2">Start From</label>
                                    <input type="month" name="transport_start_month" id="transport_start_month"
                                        class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 shadow-sm"
                                        value="{{ date('Y-m') }}">
                                </div>
                            </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-1">Used as Parent Login Password.</p>
                        </div>

                        <!-- Parent Email -->
                        <div>
                            <label for="parent_email" class="block text-sm font-medium text-gray-700 mb-2">Parent Email (Login ID)</label>
                            <input type="email" name="parent_email" id="parent_email"
                                class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 shadow-sm"
                                placeholder="parent@example.com" value="{{ old('parent_email') }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="pt-6 border-t border-gray-100 flex items-center justify-end gap-3">
                    <a href="{{ route('admin.students') }}" class="px-6 py-2.5 rounded-xl border border-gray-300 text-gray-700 font-medium hover:bg-gray-50 transition-colors">
                        Cancel
                    </a>
...
<script>
    ...
    if (parentEmailInput) {
        parentEmailInput.addEventListener('blur', function() {
            ...
            if (email && email.includes('@')) {
                // Admin context check URL
                const checkUrl = '{{ route("admin.families.check") }}';
    ...
    parentPhoneInput.addEventListener('blur', function() {
        // ... (existing parent lookup remains intact)
        const phone = this.value;
        if (phone.length >= 10) {
            fetch('{{ route("admin.students.search-parent") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                ...
                .then(data => {
                    if (data.status === 'success') {
                        parentNameInput.value = data.parent.name;
                        parentEmailInput.value = data.parent.email;

                        // Show success feedback
                        const feedback = document.createElement('p');
                        feedback.textContent = 'Parent found: ' + data.parent.name;
                        feedback.className = 'text-green-600 text-xs mt-1 font-bold';
                        feedback.id = 'parent-feedback';

                        // Remove existing feedback
                        const existing = document.getElementById('parent-feedback');
                        if (existing) existing.remove();

                        parentPhoneInput.parentNode.appendChild(feedback);
                    }
                })
    ...
    function toggleTransportFields() {
        ...
        const wrapper = document.getElementById('transport_fields_wrapper');
    ...
    function updateTransportFee(select) {
        const option = select.options[select.selectedIndex];
        const fee = option.getAttribute('data-fee');
        if (fee) {
            document.getElementById('transport_fee').value = parseFloat(fee).toFixed(2);
        }
    }
